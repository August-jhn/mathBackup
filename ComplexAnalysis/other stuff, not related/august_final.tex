...
\fbox{solutoin} $F \cong \Z_p\oplus...(\mbox{n})...\oplus\Z_p$.
...
\fbox{problem 5} Let $F$ be a field. Then the only ideals of $F$ are $F$ and $\{0\}$, where zero is taken to be the additive identity.\\
...
\fbox{individual problem} Prove if $N$ is a normal subgroup of $G$ and $[G : N] = m$, then $x^m \in N$ for all $x \in G.$ \\

\fbox{proof} Suppose $x\in N$. Then since $N$ is a group, it follows by closure that $x^m\in N$.\\
