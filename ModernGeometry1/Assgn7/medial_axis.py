...
def medial_axis(P):
    ...
    que_to_remove = {} #   {  start point (v):  end point(x)  } this is a que of edges to replace
    ...
    def filter():
        """this function deletes all of the extraneous edges"""
        new_edges = []
        for edge in edges:
            v = edge[0]
            if v in que_to_remove:
                print(edge)
                new_edges.append((que_to_remove[v], edge[1]))
            else:
    ...
    def next_polygon(polygon, pair,points_to_indices, the_point):
        new_polygon = []
        ...
        print(
            pair[0], 
            polygon[(points_to_indices[pair[0]] - 1) % len(polygon)]
            )
    ...
    def find_medial_axis(polygon ):
        ...
        if len(polygon) > 2:
            ...
            for i in range(len(polygon)):
                ...
                pairs_to_intersection_points[(p1,p2)] = x
            ...
            x = pairs_to_intersection_points[minimum]
            ...
            if number_of_vertices > 4:
                find_medial_axis(next_polygon(polygon, minimum, points_to_indices, x))
            else:
                compute_triangle_medial_axis(next_polygon(polygon, minimum, points_to_indices, x))
